...
def _span_word(tag: Callable, text: Callable, word: str, score: float,
    ...
    :param score: The word's relevance score
    ...
    """
    bg = colormap(score)
    # fg = tuple(1. - c for c in bg)
    # style = "background-color:" + _get_rgb(bg) + ";color:" + _get_rgb(fg)
    style = "color:" + _get_rgb(bg) + ";font-weight:bold;"
    with tag("span", style=style):
        text(" " + word + " ")
    text(" ")


def html_heatmap(words: List[str], scores: List[float],
    ...
    :param scores: The attribution score assigned to each word
...
def latex_heatmap(words: List[str], scores: List[float],
                  cmap_name: str = "coolwarm") -> str:
    """
